...
export class CouchbaseWrapper {
  ...
  private _query: { [index: string]: (query: couchbase.N1qlQuery, params?: {[param: string]: any} | any[]) => any };
  ...
  constructor(config: CouchbaseWrapperConfig) {
    ...
    if (config.username && config.password) {
      this._cluster.authenticate(config.username, config.password);
    }
    this._connections = {};
    this._getDoc = {};
    this._upsertDoc = {};
      ...
      if (!this._connections[bucketName]) {
        ...
        if (ba.password) {
          this._connections[bucketName] = {
            bucket: this._cluster.openBucket(ba.bucket, ba.password, cb),
          };
        } else {
          this._connections[bucketName] = {
            bucket: this._cluster.openBucket(ba.bucket, cb),
          };
        }
      }
   ...
   * @param ops
   */
  query(bucket: string, queryString: string, ops?: any): Promise<Array<any>> {
    const query = couchbase.N1qlQuery.fromString(queryString);
    if (!this._connections[bucket]) {
      throw Error(`No bucket connection for ${bucket}`);
    }
    if (ops) {
      return this._query[bucket](query, ops);
    }
